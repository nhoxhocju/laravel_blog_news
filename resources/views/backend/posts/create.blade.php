    ...
    <div class="row">
        ...
        <div class="col-lg-12 col-12">
            ...
            <div class="card card-primary">
                ...
                <!-- /.card-header -->
                <!-- form start -->
                <form role="form" action="{{ route('backend.post.store') }}" enctype="multipart/form-data" method="post">
                    @method('POST')
                    @csrf
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                {{-- <input type="button" id="lfm" data-input="thumbnail" data-preview="holder"
                                    value="Upload"> --}}
                                <input id="thumbnail" type="file" onchange="readURL(this)" class="form-control" name="thumbnail">
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="title-{{ $language->code_language }}" class="form-control" placeholder="{{ __('Enter title post') }}"
                                        name="title-{{ $language->code_language }}">
                                    <label>{{ __('Content') }}</label>
                                    <textarea class="form-control" id="{{ $language->code_language }}-ckeditor"
                                        name="content-{{ $language->code_language }}"></textarea>
                                </div>
                                @endforeach

                            </div>
                            <div class="form-group">
                                <label for="">Chuyên mục</label><br>
                                <select id="inputState" class="form-control" name="category">
                                    <option value="">Choose...</option>
                                    @foreach($categories as $cat)
                                <option value="{{ $cat->pivot->category_id }}">{{$cat->pivot->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="">Đường dẫn bài viết</label><br>
                            <input class="form-control" type="text" name="slug" value="" placeholder="{{ __('Enter slug post') }}" />
                            </div>
                        </div>
                    ...
                    <div class="card-footer">
                        <a href="{{ route('backend.post.index') }}" class="btn btn-default mr-2">{{ __('Back') }}</a>
